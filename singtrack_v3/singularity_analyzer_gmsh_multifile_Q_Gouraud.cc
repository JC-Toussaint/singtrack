...
// ou pour identifier des valeurs considéres comme purement réelles/imaginaires (proches de la précision machine).
...
// AJOUT SÉCURITÉ : expected_nodes sert désormais de garde-fou absolu lors du redimensionnement et du parsing.
Mesh load_mesh_gmsh(const std::string& msh_filename, size_t expected_nodes) {
    ...
    // Dimensionnement strict de la matrice de points basé sur le nombre attendu dicté par la solution (.sol).
    // Cela empêche l'allocation d'une matrice décalée si le fichier .msh contient des indices aberrants.
    mesh.points.resize(expected_nodes, 3);
    mesh.points.setZero(); // Initialisation préventive sécurisée
    ...
    for (size_t i = 0; i < nodeTags.size(); ++i) {
        ...
        // CORRECTION DE COHÉRENCE CRITIQUE : Si le fichier de maillage contient des indices de nœuds supérieurs 
        // ou égaux au nombre total attendu de nœuds, l'écriture provoquerait un écrasement mémoire immédiat (out-of-bounds).
        // On stoppe le programme immédiatement avec une erreur explicite.
        if (idx >= expected_nodes) {
            gmsh::finalize();
            throw std::runtime_error("CRITIQUE : Le fichier de maillage .msh reference un tag de noeud physique (" + std::to_string(tag) 
                                     + ") qui depasse la taille stricte de la solution attendue (" + std::to_string(expected_nodes) + ").");
        }
    ...
    for (size_t i = 0; i < elementTypes.size(); ++i) {
        ...
        if (type == 2) { // Type 2 dans la nomenclature Gmsh correspond à un triangle linéaire à 3 nœuds (2D)
            for (size_t j = 0; j < nodes.size(); j += 3) {
                ...
                // VÉRIFICATION DE SÉCURITÉ : On s'assure qu'aucune table de connectivité du maillage ne pointe
                // vers un nœud en dehors de la mémoire allouée (éviter les SegFaults internes dans la boucle principale).
                if (idx0 >= 0 && idx0 < (int)expected_nodes && 
                    idx1 >= 0 && idx1 < (int)expected_nodes && 
                    idx2 >= 0 && idx2 < (int)expected_nodes) {
                    mesh.triangles.push_back(Eigen::Vector3i(idx0, idx1, idx2));
                } else {
                    gmsh::finalize();
                    throw std::runtime_error("CRITIQUE : La table des éléments de surface (triangles) pointe vers un noeud hors-limites.");
                }
            }
        } 
            ...
            for (size_t j = 0; j < nodes.size(); j += 4) {
                ...
                // VÉRIFICATION DE SÉCURITÉ SUR LES TÉTRAÈDRES
                if (idx0 >= 0 && idx0 < (int)expected_nodes && 
                    idx1 >= 0 && idx1 < (int)expected_nodes && 
                    idx2 >= 0 && idx2 < (int)expected_nodes && 
                    idx3 >= 0 && idx3 < (int)expected_nodes) {
                    mesh.tetrahedrons.push_back(Eigen::Vector4i(idx0, idx1, idx2, idx3));
                } else {
                    gmsh::finalize();
                    throw std::runtime_error("CRITIQUE : La table des éléments de volume (tétraèdres) pointe vers un noeud hors-limites.");
                }
            }
        }
    }
...
std::unique_ptr<SurfaceSingularityResult> analyze_surface_singularity(int current_iter, double current_time, const Eigen::Matrix3d& ns_coords, const Eigen::Matrix3d& ns_mag) {
    ...
    // AJOUT COMMENTAIRE : Indispensable pour réaliser le changement de base 3D -> 2D (Plan tangent).
    ...
    // Utilisation d'un solveur robuste basé sur la décomposition QR.
...
int main(int argc, char* argv[]) {
    ...
    // Capture explicite de la dimension de référence dictée par les fichiers solutions (.sol)
    const size_t expected_nodes = initial_mag.rows();
    std::cout << "Nombre de noeuds attendu (donne par les solutions) : " << expected_nodes << "\n";
    ...
    Mesh mesh;
    try {
        // AJOUT SÉCURITÉ : Transmission de la taille 'expected_nodes' pour contrôle de cohérence au cœur du parsing Gmsh
        mesh = load_mesh_gmsh(msh_file, expected_nodes);
    } catch (const std::exception& e) {
        // En cas d'incohérence détectée par Gmsh, le programme s'arrête immédiatement et proprement ici.
        std::cerr << "\n[ERREUR DE COHÉRENCE GÉOMÉTRIQUE CRITIQUE]\n" << e.what() << "\nL'execution a ete stoppee.\n";
        return 1;
    }
    ...
    for (const auto& [iter, file_path] : sol_files) {
        ...
        // Garde-fou numérique et protection de la mémoire (Sécurité d'exécution) : 
        // MODIFICATION RIGOURÈSE : Si un fichier .sol courant possède un nombre de nœuds différent de notre maillage géométrique,
        // le programme est stoppé immédiatement afin d'éviter tout accès mémoire désaligné ("Segmentation fault").
        if ((size_t)mag.rows() != mesh.points.rows()) {
            std::cerr << "\n[ERREUR DE COHÉRENCE DE DONNÉES CRITIQUE]\n"
                      << "Le fichier de solution courant '" << file_path << "' contient " << mag.rows() << " noeuds.\n"
                      << "Le maillage charge attend quant a lui strictement : " << mesh.points.rows() << " noeuds.\n"
                      << "Arret immediat du programme pour des raisons de securite memoire.\n";
            return 1;
        }
