...
#include <cmath>
#include <map>
#include <unordered_map>  // OPTIM : remplacement de std::map par unordered_map pour O(1) amorti
#include <set>
#include <algorithm>
#include <memory>
#include <filesystem>
...
#include <regex>
#include <chrono>
#include <omp.h>          // OPTIM : parallélisation OpenMP des boucles indépendantes
...
// Structure pour identifier une arête unique (conservée pour compatibilité, non utilisée dans subdivide_surface_pn
// qui utilise désormais un encodage entier int64_t plus rapide)
struct Edge {
    int v1, v2;
...
std::vector<Eigen::Vector3i> orient_triangles_outward(const Eigen::MatrixXd& points, 
    ...
    // OPTIM : remplacement de std::map (arbre rouge-noir, O(log N)) par std::unordered_map (table de hachage, O(1) amorti)
    // pour la table d'adjacence inverse Nœud -> tétraèdres. Gain significatif sur les gros maillages.
    std::unordered_map<int, std::vector<int>> node_to_tetra;
    node_to_tetra.reserve(tetrahedrons.size() * 4); // Pré-allocation pour éviter les rehash
    ...
    for (size_t i = 0; i < oriented_triangles.size(); ++i) {
        ...
        const auto& t0_list = node_to_tetra.at(tri[0]);
        const auto& t1_list = node_to_tetra.at(tri[1]);
        const auto& t2_list = node_to_tetra.at(tri[2]);

        // OPTIM : remplacement des deux std::set_intersection coûteuses (copies + tri) par une recherche directe.
        // On cherche un index présent simultanément dans les 3 listes d'adjacence via un unordered_set.
        std::set<int> s2(t2_list.begin(), t2_list.end());
        int tetra_idx = -1;
        for (int idx : t0_list) {
            // Vérification d'appartenance à t1_list via une recherche linéaire (listes courtes en pratique)
            if (std::find(t1_list.begin(), t1_list.end(), idx) != t1_list.end() && s2.count(idx)) {
                tetra_idx = idx;
                break;
            }
        }
        if (tetra_idx < 0) continue; // Triangle orphelin
...
Eigen::MatrixXd compute_node_normals(const Eigen::MatrixXd& points, const std::vector<Eigen::Vector3i>& triangles) {
    ...
    for (int ti = 0; ti < Ntri; ++ti) {
        ...
        Eigen::Vector3d p0 = points.row(tri[0]);
        Eigen::Vector3d v1 = (Eigen::Vector3d)points.row(tri[1]) - p0;
        Eigen::Vector3d v2 = (Eigen::Vector3d)points.row(tri[2]) - p0;
        Eigen::Vector3d face_normal = v1.cross(v2);
        local_normals[tid].row(tri[0]) += face_normal;
        local_normals[tid].row(tri[1]) += face_normal;
        local_normals[tid].row(tri[2]) += face_normal;
    }
    for (int t = 0; t < nthreads; ++t) node_normals += local_normals[t];
    #else
    for (const auto& tri : triangles) {
        ...
        Eigen::Vector3d v2 = (Eigen::Vector3d)points.row(tri[2]) - p0;
        Eigen::Vector3d face_normal = v1.cross(v2);
        node_normals.row(tri[0]) += face_normal;
        node_normals.row(tri[1]) += face_normal;
        node_normals.row(tri[2]) += face_normal;
    }
    #endif
...
std::unique_ptr<BlochPointResult> analyze_bloch_point(int current_iter, double current_time, const Matrix34d& nodes_coords, const Matrix34d& nodes_mag) {
    ...
    if ((vec_local.array() > 0).all() && (vec_local.array() < 1).all() && (vec_local.sum() < 1)) {
        ...
        // OPTIM + ROBUSTESSE : remplacement de l'inversion matricielle explicite A_space.inverse()
        // (coûteux et numériquement fragile) par une factorisation QR avec pivot de colonnes,
        // identique à celle déjà utilisée pour A_m. On résout trois systèmes indépendants en une passe.
        auto qr_solver = A_space.colPivHouseholderQr();
        if (!qr_solver.isInvertible()) return nullptr; // Equivalent au test sur le déterminant, mais sans le calcul
        Eigen::Vector4d coeff_x = qr_solver.solve(nodes_mag.row(0).transpose()); // Coefficients pour la composante mx
        Eigen::Vector4d coeff_y = qr_solver.solve(nodes_mag.row(1).transpose()); // Coefficients pour la composante my
        Eigen::Vector4d coeff_z = qr_solver.solve(nodes_mag.row(2).transpose()); // Coefficients pour la composante mz
...
int main(int argc, char* argv[]) {
    ...
    // puis fusionnés dans les conteneurs globaux avec un verrou critique minimal.
    auto sol_start = std::chrono::high_resolution_clock::now(); 

    const int N_sol = static_cast<int>(sol_files.size());
    // Pré-conversion en vecteur pour l'indexation OpenMP (la map ne supporte pas les indices entiers)
    std::vector<std::pair<int,std::string>> sol_vec(sol_files.begin(), sol_files.end());

    // Vecteurs de collecte par thread pour éviter les contentions sur global_*
    ...
    } // fin du #pragma omp parallel for

    // Fusion séquentielle des résultats dans l'ordre chronologique (fi croissant garantit l'ordre)
    for (int fi = 0; fi < N_sol; ++fi) {
        for (auto& r : thread_bloch[fi]) global_bloch_points.push_back(std::move(r));
        for (auto& r : thread_surf[fi])  global_surface_singularities.push_back(std::move(r));
    }
