...
// ou pour identifier des valeurs considéreés comme purement réelles/imaginaires (proches de la précision machine).
...
struct Mesh {
    ...
    Eigen::MatrixXd node_normals;            // [NOUVEAUTÉ] Matrice dynamique (N lignes, 3 colonnes) stockant les normales lissées (Gouraud) aux nœuds de surface.
...
Mesh load_mesh_gmsh(const std::string& msh_filename, size_t expected_nodes) {
    ...
    for (size_t i = 0; i < nodeTags.size(); ++i) {
        ...
        size_t idx = tag - 1; // Gmsh использует une numérotation à base 1 pour les tags. On décale à base 0 pour l'indexation C++/Eigen.
...
// --- [NOUVEAUTÉ] CALCUL DES NORMALES NODALES LISSÉES (STYLE GOURAUD) ---
// Calcule pour chaque nœud de surface une normale lissée moyenne, pondérée par l'aire des faces
// triangulaires adjacentes (Gouraud shading principle). Les normales résultantes sont unitaires.
void compute_node_normals(Mesh& mesh) {
    std::cout << "Calcul des normales nodales lissees (Gouraud)..." << std::endl;
    
    // Initialisation de la matrice des normales à zéro pour tous les points du maillage
    mesh.node_normals = Eigen::MatrixXd::Zero(mesh.points.rows(), 3);
    
    // Accumulation des normales de chaque face sur ses sommets respectifs
    // Le produit vectoriel brut (v1 x v2) a une norme proportionnelle à deux fois l'aire du triangle.
    // L'ajouter directement réalise implicitement la pondération par l'aire (Gouraud style).
    for (const auto& tri : mesh.triangles) {
        Eigen::Vector3d p0 = mesh.points.row(tri[0]);
        Eigen::Vector3d p1 = mesh.points.row(tri[1]);
        Eigen::Vector3d p2 = mesh.points.row(tri[2]);
        
        Eigen::Vector3d v1 = p1 - p0;
        Eigen::Vector3d v2 = p2 - p0;
        Eigen::Vector3d face_normal = v1.cross(v2); // Non normalisé pour préserver le poids de l'aire
        
        mesh.node_normals.row(tri[0]) += face_normal;
        mesh.node_normals.row(tri[1]) += face_normal;
        mesh.node_normals.row(tri[2]) += face_normal;
    }
    
    // Normalisation finale pour rendre chaque vecteur de normale nodal unitaire
    int active_surface_nodes = 0;
    for (int i = 0; i < mesh.node_normals.rows(); ++i) {
        double norm = mesh.node_normals.row(i).norm();
        if (norm > TOL) {
            mesh.node_normals.row(i) /= norm;
            active_surface_nodes++;
        }
    }
    std::cout << "=> Normales nodales calculees avec succes pour " << active_surface_nodes << " noeuds de surface.\n";
}
    ...
    if ((vec_local.array() > 0).all() && (vec_local.array() < 1).all() && (vec_local.sum() < 1)) {
        ...
        */
        ...
        if (all_real) {
            ...
            // Cas 1 : Toutes les valeurs propres sont strictly positives -> Divergence pure (Source)
            if (r0 > 0 && r1 > 0 && r2 > 0) bp_type = "source";
            // Cas 2 : Toutes les valeurs propres sont strictly négatives -> Convergence pure (Puits / Sink)
...
// [MODIFICATION] Prends désormais en argument la matrice des normales nodales lissées pour projeter de manière plus précise.
std::unique_ptr<SurfaceSingularityResult> analyze_surface_singularity(int current_iter, double current_time, const Eigen::Matrix3d& ns_coords, const Eigen::Matrix3d& ns_mag, const Eigen::Matrix3d& ns_normals) {
    // Calcul de l'aire géométrique brute du triangle (requise pour la charge topologique)
    Eigen::Vector3d v1 = ns_coords.col(1) - ns_coords.col(0);
    Eigen::Vector3d v2 = ns_coords.col(2) - ns_coords.col(0);
    Eigen::Vector3d raw_n = v1.cross(v2);
    double n_norm = raw_n.norm();
    if (n_norm == 0) return nullptr; // Sécurité : évite les triangles plats d'aire nulle
    
    // [NOUVEAUTÉ Q] : Calcul de la surface réelle de l'élément triangulaire courant
    double triangle_area = 0.5 * n_norm;

    // [LISSAGE] Utilisation de la normale lissée moyenne au centre du triangle (Gouraud smoothing)
    // au lieu de la normale géométrique abrupte de la face.
    Eigen::Vector3d n = ns_normals.rowwise().mean(); 
    n.normalize(); // Garantit la stricte unitarité du vecteur normal combiné

    // Création d'un repère local orthonormé direct (t0, t1, n) attaché au plan tangent lissé du triangle
    ...
    if (vec_local[0] > 0 && vec_local[1] > 0 && (vec_local[0] + vec_local[1] < 1)) {
        ...
        // Reconstruction du vecteur d'aimantation interpolé au centre de la singularité
...
int main(int argc, char* argv[]) {
    ...
    // [NOUVEAUTÉ] Application du lissage des normales à la surface du domaine géométrique
    compute_node_normals(mesh);
    ...
    std::cout << "=> Temps de traitement du maillage (avec lissage) : " << mesh_duration.count() << " secondes.\n";
    ...
    for (const auto& [iter, file_path] : sol_files) {
        ...
        for (const auto& nodes_idx : mesh.tetrahedrons) {
            ...
            // Test de pré-filtrage topologique ultra-rapide (sign_check)
        ...
        for (const auto& nodes_idx : mesh.triangles) {
            ...
            for(int i = 0; i < 3; ++i) {
                ...
                s_normals.row(i) = mesh.node_normals.row(nodes_idx[i]); // [LISSAGE] Chargement des normales lissées précalculées
            }

            // [MODIFICATION] Transmission du sous-bloc de normales lissées à l'analyse de surface
