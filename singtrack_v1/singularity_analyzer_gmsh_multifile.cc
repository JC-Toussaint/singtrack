...
const std::string DIRECTORY_PATH = "."; // Dossier où se trouvent les fichiers sol*.in ('.' signifie dossier courant)
...
Eigen::MatrixXd load_magnetization(const std::string& sol_filename) {
...
Mesh load_mesh_gmsh(const std::string& msh_filename) {
    Mesh mesh;
    ...
    // Trouver dynamiquement l'index maximal pour allouer la bonne taille de matrice
    size_t max_tag = 0;
    for (size_t tag : nodeTags) {
        if (tag > max_tag) max_tag = tag;
    }
    mesh.points.resize(max_tag, 3);
...
std::vector<Eigen::Vector3i> orient_triangles_outward(const Eigen::MatrixXd& points, 
    ...
    std::cout << "Structure de recherche des tétraèdres en cours... \n";
...
std::unique_ptr<BlochPointResult> analyze_bloch_point(int current_iter, const Matrix34d& nodes_coords, const Matrix34d& nodes_mag) {
    Eigen::Matrix3d A_m;
    ...
    if ((vec_local.array() > 0).all() && (vec_local.array() < 1).all() && (vec_local.sum() < 1)) {
        ...
        return std::make_unique<BlochPointResult>(BlochPointResult{
            current_iter, sol_cartesian, Eigen::Vector3d(curl_x, curl_y, curl_z), eigvals, bp_type
        });
    }
    return nullptr;
}
...
std::unique_ptr<SurfaceSingularityResult> analyze_surface_singularity(int current_iter, const Eigen::Matrix3d& ns_coords, const Eigen::Matrix3d& ns_mag) {
    ...
    if (vec_local[0] > 0 && vec_local[1] > 0 && (vec_local[0] + vec_local[1] < 1)) {
        ...
        return std::make_unique<SurfaceSingularityResult>(SurfaceSingularityResult{
            current_iter, sol_cartesian, curl_n, eigvals, surf_type, polarity
        });
    }
    return nullptr;
}
...
// --- MAIN ---
int main() {
    // 1. CHARGEMENT ET CORRECTION DU MAILLAGE (EN PREMIER)
    std::cout << "Chargement du maillage initial : " << MSH_FILE << "...\n";
    Mesh mesh;
    try {
        mesh = load_mesh_gmsh(MSH_FILE);
    } catch (const std::exception& e) {
        std::cerr << "Erreur critique de maillage : " << e.what() << "\n";
        return 1;
    }
    
    std::cout << " -> Noeuds chargés : " << mesh.points.rows() << "\n";
    std::cout << " -> Tétraèdres (Volume) : " << mesh.tetrahedrons.size() << "\n";
    std::cout << " -> Triangles (Surface) : " << mesh.triangles.size() << "\n";

    // Réorientation des éléments surfaciques par rapport au volume
    mesh.triangles = orient_triangles_outward(mesh.points, mesh.tetrahedrons, mesh.triangles);

    // 2. RECHERCHE ET CONFIGURATION DES FICHIERS SOLUTIONS
    std::vector<std::pair<int, std::string>> sol_files;
    std::cout << "\nRecherche des fichiers solutions dans '" << DIRECTORY_PATH << "'...\n";
    ...
    if (sol_files.empty()) {
        std::cerr << "Aucun fichier solution (*.sol) trouvé dans le dossier.\n";
        return 1;
    }

    // Tri par ordre d'itération
    std::sort(sol_files.begin(), sol_files.end());
    std::cout << sol_files.size() << " fichiers solutions trouvés et triés chronologiquement.\n";

    // Conteneurs pour l'accumulation globale
    std::vector<BlochPointResult> global_bloch_points;
    std::vector<SurfaceSingularityResult> global_surface_singularities;

    // 3. BOUCLE DE TRAITEMENT SUR LES SOLUTIONS
    for (const auto& [iter, file_path] : sol_files) {
        std::cout << "\n---------------------------------------------\n";
        std::cout << "Traitement de : " << file_path << " (Iteration: " << iter << ")...\n";

        Eigen::MatrixXd mag;
        try {
            mag = load_magnetization(file_path);
        } catch (const std::exception& e) {
            std::cerr << "Échec de lecture pour " << file_path << " (" << e.what() << "), ignoré.\n";
            continue;
        }

        // VÉRIFICATION DE LA STRUCTURE (Nombre de lignes de la solution == Nombre de nœuds du maillage)
        if (mag.rows() != mesh.points.rows()) {
            std::cerr << "Incohérence structurelle fatale dans " << file_path 
                      << " (Lignes Sol: " << mag.rows() << " vs Nœuds Maillage: " << mesh.points.rows() << "). Fichier ignoré.\n";
            continue;
        }
        ...
        for (const auto& nodes_idx : mesh.tetrahedrons) {
            ...
            if (sign_check(t_mag, 0) && sign_check(t_mag, 1) && sign_check(t_mag, 2)) {
                auto res_bp = analyze_bloch_point(iter, t_coords, t_mag);
                if (res_bp) {
        ...
        for (const auto& nodes_idx : mesh.triangles) {
            ...
            auto res_surf = analyze_surface_singularity(iter, s_coords, s_mag);
            if (res_surf) {
    ...
    if(f_vol.is_open()) {
        f_vol << std::left  << std::setw(10) << "iter"
              << std::right << std::setw(12) << "x"
              << std::setw(12) << "y"
              << std::setw(12) << "z"
              << "  type\n";
        f_vol << std::fixed << std::setprecision(3);
        for (const auto& bp : global_bloch_points) {
            f_vol << std::right
                  << std::setw(10) << bp.iter
                  << std::setw(12) << bp.pos.x()
                  << std::setw(12) << bp.pos.y()
                  << std::setw(12) << bp.pos.z()
                  << "  " << std::left << bp.type << '\n';
        }
    }
    ...
    if(f_surf.is_open()) {
        f_surf << std::left  << std::setw(10) << "iter"
               << std::right << std::setw(12) << "x"
               << std::setw(12) << "y"
               << std::setw(12) << "z"
               << "  type\n";
        f_surf << std::fixed << std::setprecision(3);
        for(const auto& ss : global_surface_singularities) {
            f_surf << std::right
                   << std::setw(10) << ss.iter
                   << std::setw(12) << ss.pos.x()
                   << std::setw(12) << ss.pos.y()
                   << std::setw(12) << ss.pos.z()
                   << "  " << std::left << ss.type << '\n';
        }
    }

    std::cout << "Analyses multi-fichiers terminées avec succès.\n";
    return 0;
}